...
Model: meta-llama/Llama-3.3-70B-Instruct   (70B dense, ≈40 GB at 4-bit nf4)
...
Usage:
    python riskguardian_hf_llama_classifier.py                 # validate (40 rows) + classify the 70 test rows
    python riskguardian_hf_llama_classifier.py --no-validate   # skip validation, just classify + submit
    python riskguardian_hf_llama_classifier.py --dry-run       # no model load: just print a sample prompt
    HF_MODEL=... python riskguardian_hf_llama_classifier.py    # override the model id
...
MODEL_ID = os.environ.get("HF_MODEL", "meta-llama/Llama-3.3-70B-Instruct")
