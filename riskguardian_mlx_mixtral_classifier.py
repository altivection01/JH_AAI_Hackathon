...
Model: mlx-community/Mixtral-8x22B-Instruct-v0.1-4bit   (141B sparse-MoE (≈39B active), ≈78 GB at 4-bit)

Requires: macOS on Apple silicon + `pip install mlx-lm` (plus numpy, pandas). ~≈78 GB of the
128 GB unified memory holds the weights, leaving ample room for the KV-cache.

Usage:
    python riskguardian_mlx_mixtral_classifier.py                 # validate (40 rows) + classify the 70 test rows
    python riskguardian_mlx_mixtral_classifier.py --no-validate   # skip validation, just classify + submit
    python riskguardian_mlx_mixtral_classifier.py --dry-run       # no model load: just print a sample prompt
    HF_MODEL=mlx-community/... python riskguardian_mlx_mixtral_classifier.py   # override the model id
...
MODEL_ID = os.environ.get("HF_MODEL", "mlx-community/Mixtral-8x22B-Instruct-v0.1-4bit")
