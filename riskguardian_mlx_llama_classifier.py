...
Writes:
    submission2.llama-3.3-70b.csv   <- predictions (pure GenAI / FAQ #8; no classical fallback)

Model: mlx-community/Llama-3.3-70B-Instruct-4bit   (70B dense, ≈40 GB at 4-bit)
...
Usage:
    python riskguardian_mlx_llama_classifier.py                 # validate (40 rows) + classify the 70 test rows
    python riskguardian_mlx_llama_classifier.py --no-validate   # skip validation, just classify + submit
    python riskguardian_mlx_llama_classifier.py --dry-run       # no model load: just print a sample prompt
    HF_MODEL=mlx-community/... python riskguardian_mlx_llama_classifier.py   # override the model id
...
MODEL_ID = os.environ.get("HF_MODEL", "mlx-community/Llama-3.3-70B-Instruct-4bit")
OUT_PATH = "submission2.llama-3.3-70b.csv"
