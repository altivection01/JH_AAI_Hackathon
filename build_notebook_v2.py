...
v2: reproduces the *structure* of the committed
`RiskGuardian_Cyber_Risk_Assessment.ipynb` (reflowed paragraphs, brand-green
numbering, white-bg logo swap, and the cross-model agreement chart cells).
It emits UN-EXECUTED cells: cell outputs and embedded chart images are absent
and only appear after the notebook is run. The committed .ipynb remains the
...
# ── v2 post-process: reproduce the committed notebook's hand-edits ────────
...
_REFRAME = " This holdout doubles as an **assumption test**: the engineered prompt hard-codes the discovered two-sub-corpus structure, so the per-quadrant accuracy (all four quadrants at 1.00) is evidence the *structure itself* is right — not merely that the prompt got lucky on a handful of rows."

# A small number of cells in the committed notebook carry HAND-EDITS that the
# four generic transforms (logo-swap / reframe / reflow / green-numbers) do not
# reproduce. The generic `_reflow` corrupts ASCII-art code fences and does not
# join the specific blockquote/fence lines a human merged by hand. Rather than
# guess, we reproduce those committed cells with small, content-keyed overrides
# (each is a no-op if the source text ever changes). See the per-cell notes.
#
#  - "Part I — Research" cell:  left UN-reflowed in the committed notebook
#    (its list-item continuations are intentionally kept wrapped).
#  - "Part II — Reference architecture" cell: reflow the prose but PRESERVE the
#    ASCII reference-architecture diagram inside the ``` fence.
#  - "Part XIV — submission" cell: a human merged exactly two blockquote lines.
#  - logo / "Part XVII" cell: trimmed to just the (white-bg) logo <div>, with a
#    new alt text; the Part XVII intro prose below it was removed by hand.
#  - holdout reframe cell: gains the assumption-test reframe + a trailing newline.

def _reflow_fence_aware(src):
    """Like `_reflow`, but ``` fenced blocks are handled specially: an
    ASCII-art fence (indented body) is preserved verbatim, while a plain
    command/text fence (flush-left body) is reflowed like prose. Outside
    fences this is identical to `_reflow`."""
    lines = src.split('\n'); out = []; buf = []
    def flush():
        if buf: out.append(' '.join(x.strip() for x in buf)); buf.clear()
    i = 0
    while i < len(lines):
        ...
        if l.lstrip().startswith('```'):
            ...
            while i < len(lines) and not lines[i].lstrip().startswith('```'):
                fl.append(lines[i]); i += 1
            if i < len(lines): fl.append(lines[i])  # closing fence
            body = fl[1:-1]
            if any(b[:1] in (' ', '\t') for b in body if b.strip()):
                out.extend(fl)                       # ASCII-art: preserve
            else:                                    # plain fence: reflow body
                out.append(fl[0]); jb = []
                def jflush():
                    if jb: out.append(' '.join(x.strip() for x in jb)); jb.clear()
                for b in body:
                    if b.strip() == '': jflush(); out.append('')
                    else: jb.append(b)
                jflush(); out.append(fl[-1])
            i += 1; continue
        if l.strip() == '': flush(); out.append('')
        elif _is_boundary(l): flush(); out.append(l)
        elif _is_list_item(l): flush(); buf.append(l)
...
_LOGO_ONLY = ('<div align="center"><img src="data:image/png;base64,' + _white_b64 +
              '" alt="the_plotly_thickens — RiskGuardian (JHU AAI Hackathon)" '
              'width="320" /></div>')
_BQ_OLD = ("> synthetic tabular `risk_class` modeled above — so the **graded "
           "`submission.csv` is produced by the\n> pure-GenAI predictor in Part "
           "XVII** (FAQ #8) from the real `combined_risk_*.csv` files. Part XVI fits a")
_BQ_NEW = ("> synthetic tabular `risk_class` modeled above — so the **graded "
           "`submission.csv` is produced by the pure-GenAI predictor in Part "
           "XVII** (FAQ #8) from the real `combined_risk_*.csv` files. Part XVI fits a")

# Apply reframe + logo-swap + reflow + green-numbers to every markdown cell.
for c in cells:
    if c.cell_type != "markdown":
        continue
    s = c.source if isinstance(c.source, str) else "".join(c.source)

    # logo / Part XVII cell -> committed keeps ONLY the (white) logo <div>.
    if 'the_plot_thickens' in s and 'Part XVII — GenAI predictor' in s:
        c.source = _LOGO_ONLY
        continue

    # "Part I — Research" cell -> committed leaves it UN-reflowed (logo swap /
    # green-numbers are still safe no-ops here).
    if s.lstrip().startswith('## ') and 'Part I — Research: the state of the art' in s:
    ...
    s = _swap_alpha_logo(s)
    if 'held-out validation above' in s and 'assumption test' not in s:
        s = s + _REFRAME + "\n"          # committed cell keeps a trailing newline
    s = _reflow_fence_aware(s)
    s = _green_numbers(s)
    if _BQ_OLD in s:                     # Part XIV: human-merged blockquote pair
        s = s.replace(_BQ_OLD, _BQ_NEW)
    c.source = s

# Insert the cross-model agreement chart cells right after the read-only
...
_blocks = _vjson.loads(_V2_BLOCKS_JSON)
for _i, _c in enumerate(cells):
    _s = _c.source if isinstance(_c.source, str) else "".join(_c.source)
    if _c.cell_type == "code" and 'Read-only diagnostic: classical baseline vs LLM' in _s:
        _md = nbf.v4.new_markdown_cell(_blocks['chart_md_main'])
        _cd = nbf.v4.new_code_cell(_blocks['chart_code'])
